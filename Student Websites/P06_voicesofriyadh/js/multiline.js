...
function makeLineChart(dataset, xName, yNames) {
	...
	*/
	...
	chart.formatAsDate = d3.time.format("%a at %I%p");
    ...
    chart.update = function () {
		...
		chart.objs.axes.g.selectAll(".xtext")
			.attr("y",0)
			.attr("x",10)
 ...
 //               cY.objs.line.g.style("display","none");
				cY.objs.line.g.style("stroke-dasharray", ("2, 2"));
				cY.objs.line.g.style("opacity",".3")
    ...
    chart.bind = function (bindOptions) {
        function getOptions() {
            ...
            if (bindOptions.margin) {
                ...
                chart.margin = {top: 15, right: 20, bottom: 70, left: 50};
            }
...
//			.attr("style", "outline: solid black;")
    ...
    chart.render = function () {
        ...
        function mouseHover() {
            ...
            chart.objs.tooltip.select(".tooltip .year").text("Hour: " + chart.xFormatter(chart.xFunct(d)));
			d3.select('#hoveroutput').text("Here's the hover output: " + chart.xFormatter(chart.xFunct(d)));
			inputValue = chart.xFormatter(chart.xFunct(d));
			
			//this pushes the hover value into the map updater
			rawdate = chart.xFunct(d);
			var formatter2 = d3.time.format("%a at %I%p");
			mapUpdate(formatter2(rawdate));
        }
    };
    return chart;
}

function mapUpdate(inputdate){
	d3.selectAll(".prayerHexagon")
		.attr("fill", function(d, i) { return prayerColor(grabprayer[i][inputdate]); })
		.attr("stroke",function(d, i) { return prayerStroke(grabprayer[i][inputdate]); });
	d3.selectAll(".transportationHexagon")
		.attr("fill", function(d, i) { return transportationColor(grabtransportation[i][inputdate]); })
		.attr("stroke",function(d, i) { return transportationStroke(grabtransportation[i][inputdate]); });
	d3.selectAll(".entertainmentHexagon")
		.attr("fill", function(d, i) { return entertainmentColor(grabentertainment[i][inputdate]); })
		.attr("stroke",function(d, i) { return entertainmentStroke(grabentertainment[i][inputdate]); });
    d3.selectAll(".saudiHexagon")
		.attr("fill", function(d, i) { return saudiColor(grabsaudi[i][inputdate]); })
		.attr("stroke",function(d, i) { return saudiStroke(grabsaudi[i][inputdate]); });
	d3.selectAll(".localHexagon")
		.attr("fill", function(d, i) { return localColor(grablocal[i][inputdate]); })
		.attr("stroke",function(d, i) { return localStroke(grablocal[i][inputdate]); });
	d3.selectAll(".visitorHexagon")
		.attr("fill", function(d, i) { return visitorColor(grabvisitor[i][inputdate]); })
		.attr("stroke",function(d, i) { return visitorStroke(grabvisitor[i][inputdate]); });
}
